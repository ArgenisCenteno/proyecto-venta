...
class SubCategoriaController extends Controller
{
            ...
            return DataTables::of($subcategorias)
                ->addColumn('actions', function($row) {
                    return '<a href="'.route('subcategorias.edit', [$row->id]).'" class="btn btn-info"><span class="material-icons">edit</span></a>
                            <form action="'.route('subcategorias.destroy', [$row->id]).'" method="POST" style="display:inline;" class="btn-delete2">
    ...
    {
        $proveedor = SubCategoria::where('id', $id)->first();
        ...
        $proveedor->delete();
        Alert::success('¡Éxito!', 'Subcategoria eliminada exitosamente')->showConfirmButton('Aceptar', 'rgba(79, 59, 228, 1)');
        return redirect()->route('subcategorias.index');
    }
}
